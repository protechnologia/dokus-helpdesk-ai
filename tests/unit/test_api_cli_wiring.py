...
def test_help_lists_the_command_tree() -> None:
    """`helpdesk --help` → exit 0 and the subcommand section (Typer stayed in tree mode)."""
...
def test_version_command_runs() -> None:
    """`helpdesk version` → exit 0 and the package name in the output (entry point wiring works)."""
